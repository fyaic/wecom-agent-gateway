...
2026-09-09 执行更新：v3 24h 实际窗口已经结束，原自动报告因静默 journal 门失败而保持 failed；
独立单调时长和同代际日志补证支持有限低负载观察，不追认为自动认证。真实私聊收尾九项通过，
日常 Mac 单实例已恢复，实验 VM/防休眠已停止。见[收尾证据](reviews/linux-soak-closure-2026-09-09.md)。
本轮两个可靠性缺口已修复：未来采集器静默误判、可选停止卡终态淘汰；首页/案例证据复核已完成，
主审补事务回滚测试后完整 CI 41 文件 / 423 项通过。下一步优先补已有原生视频/引用和各 Kernel 最新真实证据；
没有可用客户端入口或自有凭据时保持未验，不靠更多测试工具或改 Agent 推理替代。
不再默认重复 24h 准备，不增加卡片主题、模型管理、新 IM 或全能 Agent 产品层。
此前日期段落为历史决策记录，不是仍在运行的任务。

2026-09-08 13:03 执行更新：旧 v2 24h 报告已保留，不追认为新版认证；修复后的独立 v3 窗口实际启动，
最早 9 月 9 日 13:03:11（上海时区）后检查完整报告。Gateway 未重启、未更新，仍单 Bot。
下一步是按新资源错误/日志/单调时长判据完成观察、真实客户端收尾与日常服务恢复，不再重复准备新窗口。
具体时间、hash、旧报告指标及限制见[实际记录](reviews/linux-real-recovery-2026-09-07.md#新-v3-独立窗口运行中)。

2026-09-08 并行复核发现长测资源探针存在确定性的假通过路径，需要修正现有验收门，而非扩建测试平台。
当前窗口保留、不热改；旧报告不能追溯中途 spool 读取失败，严格认证须用修复后独立完整窗口。
同时拆清公开案例的 App Server/SDK 与历史/当前证据。主审合并回归后自动继续，
不要求用户重复代发测试消息；参见[状态](status.md)、[长测复核](reviews/soak-closure.md)。

2026-09-07 Linux 真实实验已发现明确主线缺陷：已持久接受的文本/文件在 90 秒断网后进入 dead，
尽管连接恢复仍未送达。已修复已知离线时的投递尝试预算并通过确定性与真实复验：
不重启 Gateway 的出站阻断恢复、两条真实文本/文件客户端可见、队列与 spool 归零。
24h 实际窗口已于 9 月 7 日 12:37:55（上海时区）启动，下一步是自动核验完整报告及恢复日常服务，
不是另开发测试工具；详见[真实故障记录](reviews/linux-real-recovery-2026-09-07.md)。不追加卡片或模型管理功能。

2026-09-07 执行更新：干净安装门、维护者新目录真实 Pi 接入（含 Bot 注册、多轮、主动通知、重启续接）
已经分别通过。下一步以实际 Linux/systemd 环境和真实失败恢复补证据，不再重复开发验收器。
依赖升级已触发 Node 最低版本和 Codex SDK 停止逻辑修正；Claude 真实登录与其他 runtime 的当次失败
必须独立保留，不能用历史成功覆盖。参见[新环境实测](reviews/fresh-real-onboarding-2026-09-07.md)。

2026-09-05 并行专项收口：[主审记录](reviews/mainline-integration.md)汇总传输 ACK、Claude 会话隔离和
soak 证据质量修复。无缓存安装门已通过；后续继续补真实客户端与单实例运行证据，不扩展模型/卡片主题。

2026-09-05 会话回顾：[产品目标与防偏航基线](product-intent.md)将用户反复强调的边界、既往偏差和
需求—证据对应固定下来。当前推进独立的无缓存安装/公开 CLI 验收；它不改变 M3.0/M3.1 真实证据尾项，
也不把跨语言接口或新 IM 提升为必交付范围。

2026-09-05 中间层专项更新：[社区对照与执行顺序](middleware-community-review.md)补充四个近似项目和
近期官方/社区 Issue，明确已发布与未合并、声明与实测的区别。新增只读 `gateway:status` 和诊断隐私回归；
接下来按可靠性证据 → 干净环境安装 → 外部协议 ADR 推进，不扩展模型管理或主题卡片。

2026-09-05 上手体验专项更新：下一阶段的社区入口以[首次使用体验审计](onboarding-review.md)为补充参考。
已提供无凭据 demo、最小配置生成和独立 Agent 两轮检查；真实 Pi/Codex 本轮通过，Kimi 当前认证失败。
先降低理解/配置成本并明确前置条件，再考虑可分发安装包；不以增加卡片主题替代主线，也不覆盖下列未完成证据。
...
- 在真实 Linux/systemd 主机分别验收受控进程重启恢复和不少于 24 小时的无重启稳定窗口，覆盖日志轮转、
  磁盘水位和健康采集。soak schema v3 对缺失指标、资源探针错误、采样空洞或窗口内进程代际变化拒绝认证；重启恢复
  成功后必须重新开始稳定窗口，不能拿同一份失败报告充当恢复失败或稳定认证。
...
| 4    | M3.1-A Linux/systemd soak + 宿主网络故障   | M3.0 | 验收器、deployment、脱敏报告 schema       | 入口完成；待独立 Linux 主机真实运行满 24h          |
