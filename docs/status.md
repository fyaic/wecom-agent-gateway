...
首个窗口已自然结束：86,400,047ms、2,877 次采样，无观测到的重启/健康失败/积压。
旧 schema v2 自报通过，但缺少可追溯的中间资源探针错误证据，不作为严格认证。
已保留旧报告，并启动修复后的独立 v3 24 小时窗口；**最早 9 月 9 日 13:03:11（上海时区）后**
核验完整报告。仅更换独立采集入口，Gateway 与单 Bot 所有权未改变，自动跟进已同步。
本轮完整 CI 仍为 40 文件 / 389 项通过；严格长测和最终客户端收尾尚未完成。
见[完整窗口记录](reviews/linux-real-recovery-2026-09-07.md#新-v3-独立窗口运行中)。

## 并行复核与当前阻塞（2026-09-08）

三个 sub-agent 分别复核 Adapter 证据、投递/卡片边界与长测验收器；主 Agent 负责交叉审查、整合及回归。
案例页已拆清 Codex App Server 与 SDK、历史成功与最近复测，原生 ask-user 自动化不冒充真实验收。
停止动作不再等待卡片更新 ACK，扣住 ACK 的确定性回归已补齐；离线排队旧停止卡的终态撤销仍待修复，
不伪造 delivered 或用正常过期污染 dead 告警。见[投递复核](reviews/delivery-closure.md)。
长测复核发现中途 spool 读取错误可能被有效整数哨兵掩盖，另有空 journal 证据不足的问题。
修复在独立工作树进行，不修改当前 Linux 服务或采集进程。
主审整合后完整 CI 为 40 文件 / 389 项通过；应用回退、示例与生成器的控制卡默认值也已统一关闭。
当前窗口即使旧报告为 passed，也不能证明修复后的资源采样有效性；保留旧证据，并以修复后的完整新窗口
关闭严格验收，不拼接时长。见[长测复核](reviews/soak-closure.md)。

## Linux 真实运行与失败恢复（2026-09-07）

专属 Linux/systemd 已完成真实 Pi/Bot 私聊与受控重启续接，两轮九项 ingress 检查和客户端回复通过。
随后客体网卡断开暴露投递缺陷：已 queued 的文本/文件在约 15 秒内耗尽五次预算。已修复并新增六项回归，
主审全量 40 文件 / 382 项通过。真实复验已证明跨 VM 重启保留排队内容，以及不重启 Gateway 的
出站网络阻断恢复；文本/文件均客户端可见，pending/leased/dead/spool 归零。失败轮次完整保留。
首个旧门窗口已结束，独立 v3 窗口进行中，详见页首交接状态；不能先写严格认证通过。
见[真实故障记录](reviews/linux-real-recovery-2026-09-07.md)。
...
本轮限定于传输可靠性、Claude Adapter 会话/取消边界和 Linux 验收器可信度；不增加模型管理、卡片主题或新 IM。
已修复非结构化错误误触发流过期降级、Claude 并发会话所有权/终态/流式串单，以及 soak 缺失采样或指标假通过。
主审另修复真实验收误用旧 `.env` Adapter 的问题，要求显式传入运行时兼容 ID。
各项证据、真实私聊基线和未完成边界见[合并审查记录](reviews/mainline-integration.md)。
真实私聊基线回复正确，九项机器检查通过，但最终回复耗时 37.206 秒，不能据此宣称低延迟端到端体验。
Claude 自有登录、原生视频/引用真实回调、Linux 24h 与宿主断网认证仍未完成。

## 需求回顾与无凭据安装门（2026-09-05）

[需求基线](product-intent.md)记录主线约束、既往偏差和未覆盖项。新增 `test:first-run` 直接运行公开 CLI
五阶段，拒绝已有配置/数据目录；本机新依赖 store 安装和命令链通过。新增 macOS/Linux 无缓存安装 CI 门，
验收范围见[上手审计](onboarding-review.md)。不新增真实 Bot、模型或媒体认证。

## 中间层可靠性与社区观察（2026-09-05）

新增 `gateway:status`，只读本地观测并区分健康、运行中、失败投递与不可用；Doctor 不再输出
外部 Adapter 的原始身份、健康详情或异常。本轮新增 16 项自动化用例；完整本地 CI 为 36 文件 / 281 项通过。现有真实服务只读快照健康，
无工作/投递积压或 dead。未重启 Bot、未新增客户端收发认证，也不改变下方未完成证据。
操作见[运行排障](gateway-status.md)，研究与下一阶段退出条件见[社区中间层对照](middleware-community-review.md)。

## 首次使用体验收口（2026-09-05）

新增无凭据六项链路演示、五类最小配置生成、独立真实 Agent 两轮检查；修复注册工具的单 Bot
所有权与 ACL 追加行为。中英文首页、接入指南、日常使用配方、示例导航与上手反馈入口同步整理。
本轮真实 Pi / Codex 两轮和会话续接通过；Kimi 当前认证类失败，OpenClaw 未复测。
这些是本地 Agent 证据，不冒充本轮新增企业微信端到端认证。
详细结果、问题清单和后续优先级见[首次使用体验审计](onboarding-review.md)。
...
| 2026-09-01 | 私聊审批拒绝                     | 通过   | 原发送者点击拒绝；SQLite 只结算一次，Kernel 未启动写工具，唯一测试标题查询为 0 条                                     |
...
| 2026-09-02 | Codex 依赖升级 / App Server 实测 | 通过   | SDK 依赖为 0.151.0；真实测试走 App Server，不认证 SDK 两轮；首文本 8.468s/2.528s、总计 9.077s/2.919s，同 session 恢复 |
...
| 2026-09-02 | 单 Bot 双进程与强杀恢复          | 通过   | 第二进程 1 秒内在 Store/Kernel/SDK 前退出；原服务保持 ready；强杀 owner 后新 PID 回收锁并重新鉴权，Outbox 零积压      |
...
- 真实图片输入与输出上传/发送均已通过；Gateway 自管耐久 spool、媒体 outbox、完整性、配额、孤儿回收
  和重启恢复已完成自动化。2026-09-04 进一步在授权私聊验证“源文件删除 → 隔离数据库/spool 重建 →
  真实附件可见”，随后恢复正式服务并以普通入站机器九项全通过确认链路健康。该轮也发现旧 smoke 会以
  第二条同 Bot WebSocket 抢占正式连接，现已接入与正式 Gateway 相同的 owner lock 并 fail closed。
  宿主机断网或真实上传失败重试仍待独立窗口验收；恶意内容扫描仍待部署策略。
...
- Codex adapter 只接受 `read-only + never` 或 `write/destructive + required`，并在 `approved` 前绝不
  ...
  1.4 秒执行成功、29.9 秒完成，测试待办随后删除并复查归零。2026-09-01 又完成真实拒绝、15 秒
  过期、Gateway `SIGKILL` 后重启和两类陈旧卡点击：三种失败路径均没有
  `runtime_tool_lifecycle=started`，唯一测试标题跨全部状态分页查询均为零。过期回调还暴露并修复了
  完成态降级文案丢失语义的问题；官方更新接口不接受无链接 `text_notice` 时，禁用投票卡现在仍
  明确显示“操作已失效”，不再误报“已完成”。Kernel turn 先结束时继续回收孤立审批。
