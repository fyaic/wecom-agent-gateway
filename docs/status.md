...
| Pi 原生 ask-user 交互桥                         | M2.2 完成，私聊 select/input 通过 | select/confirm/input/editor；native response；live resume；限定文本回复     |
| 单选卡片可读性与颜色语义                        | 私聊真实通过                      | 完整标签、无首项偏置、显式 action style、禁用完成态、重复回调幂等           |
...
- 155 项 deterministic tests 已通过；本机 Pi `0.84.2` 加载仓库示例后真实产生 select request，接受
  ...
  单选与限定文本输入已完成真实闭环；下一步只需在授权测试群复测交互 ACL。
