...
> [!NOTE]
> 第一次接入请先走 [`15 分钟接入指南`](getting-started.md)。本文是维护者的完整真实验收手册与证据记录，
> 包含多 Kernel、媒体、群聊、故障和性能路径，不要求新用户一次执行全部内容。
...
```bash
pnpm test:m3-quote-approval
```

它覆盖私聊/群聊引用文字、引用媒体、全参考 Adapter 引用映射、批准/拒绝、过期、启动时中断、
陈旧卡片不可复活、Codex 拒绝后工具零执行，以及 SQLite 跨进程状态。测试不需要 Bot Secret、
Codex 登录或企业微信客户端。任何一项失败时停止真实验收，不得用手工点击结果覆盖回归失败。
...
该记录证明批准路径、恰好一次观察结果和清理已通过。

2026-09-01 在同一授权私聊完成剩余真实矩阵：拒绝在 SQLite 中只结算一次且没有工具启动；另一请求
在 15.002 秒后过期，随后点击旧批准按钮没有恢复 Agent 或执行工具；第三个请求在审批卡送达后强杀
Gateway，新进程启动后点击旧批准按钮同样只得到失效结果。三个唯一测试标题均通过 `wecom-cli todo
list` 跨 `proceed` / `finished` 两种状态分页查询为零，因而没有清理对象。验证中发现官方卡片更新接口
会拒绝没有链接的 `text_notice`，Transport 必须退回禁用交互卡；旧实现把所有终态写成“已完成”，
会掩盖失败语义。当前回退按结果显示“已批准 / 已拒绝 / 已失效 / 正在停止 / 已结束”，真实过期旧卡
已确认原位显示“操作已失效”。该 UI 回退只负责可见状态，SQLite 中的 Broker 决定仍是唯一授权事实。
